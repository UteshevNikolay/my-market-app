...
class CartsControllerIntegrationTest {
    ...
    void buy_createsOrder() throws Exception {
        ...
        MvcResult buyResult = mockMvc.perform(post("/buy"))
                .andExpect(status().is3xxRedirection())
                .andExpect(header().string("Location", org.hamcrest.Matchers.matchesPattern(".*/orders/\\d+\\?newOrder=true")))
                .andReturn();
        ...
        @SuppressWarnings("unchecked")
        List<ItemDto> cartItems = (List<ItemDto>) cartResult.getModelAndView().getModel().get("items");
